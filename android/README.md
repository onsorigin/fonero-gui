...
# Base environnement
        ...
        cd fonero/utils/build_scripts
        docker build -f android32.Dockerfile -t fonero-android .
        cd ..

# Build GUI

        cd android/docker
        docker build -t fonero-gui-android .
        docker create -it --name fonero-gui-android fonero-gui-android bash

# Get the apk

        docker cp fonero-gui-android:/opt/android/fonero-gui/build/release/bin/bin/QtApp-debug.apk .
...
- Using ADB (Android debugger bridge) : 
  ...
  The only place where we are allowed to play is `/data/local/tmp`. So : 
        ...
        adb  push /opt/android/fonero-gui/build/release/bin/bin/QtApp-debug.apk /data/local/tmp
