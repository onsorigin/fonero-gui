...
#include "wallet/wallet2_api.h" // we need to have an access to the Fonero::Wallet::Status enum here;
...
public:
    ...
    enum Status {
        Status_Ok       = Fonero::Wallet::Status_Ok,
        Status_Error    = Fonero::Wallet::Status_Error,
        Status_Critical = Fonero::Wallet::Status_Critical
    };

    Q_ENUM(Status)

    enum ConnectionStatus {
        ConnectionStatus_Connected       = Fonero::Wallet::ConnectionStatus_Connected,
        ConnectionStatus_Disconnected    = Fonero::Wallet::ConnectionStatus_Disconnected,
        ConnectionStatus_WrongVersion    = Fonero::Wallet::ConnectionStatus_WrongVersion
    };
...
private:
    ...
    Wallet(Fonero::Wallet *w, QObject * parent = 0);
    ~Wallet();
private:
