...
class WalletListenerImpl : public  Fonero::WalletListener
{
public:
...
{
    QFuture<Fonero::Wallet::ConnectionStatus> future = QtConcurrent::run(m_walletImpl, &Fonero::Wallet::connected);
    QFutureWatcher<Fonero::Wallet::ConnectionStatus> *connectionWatcher = new QFutureWatcher<Fonero::Wallet::ConnectionStatus>();

    connect(connectionWatcher, &QFutureWatcher<Fonero::Wallet::ConnectionStatus>::finished, [=]() {
        QFuture<Fonero::Wallet::ConnectionStatus> future = connectionWatcher->future();
...
{
    Fonero::PendingTransaction * ptImpl = m_walletImpl->createTransaction(
                dst_addr.toStdString(), payment_id.toStdString(), amount,
                static_cast<Fonero::PendingTransaction::Priority>(priority));
...
{
    Fonero::PendingTransaction * ptImpl = m_walletImpl->createTransaction(
                dst_addr.toStdString(), payment_id.toStdString(), Fonero::optional<uint64_t>(),
                static_cast<Fonero::PendingTransaction::Priority>(priority));
...
UnsignedTransaction * Wallet::loadTxFile(const QString &fileName)
{
    ...
    Fonero::UnsignedTransaction * ptImpl = m_walletImpl->loadUnsignedTx(fileName.toStdString());
...
{
    return QString::fromStdString(Fonero::Wallet::genPaymentId());
}
...
{
    return QString::fromStdString(m_walletImpl->getDefaultDataDir()) + "/bitfonero.log";
}

QString Wallet::getWalletLogPath() const
{
    return QCoreApplication::applicationDirPath() + "/fonero-wallet-gui.log";
}

Wallet::Wallet(Fonero::Wallet *w, QObject *parent)
    : QObject(parent)
    , m_walletImpl(w)
    , m_history(nullptr)
...
Wallet::~Wallet()
{
    ...
    delete m_history;
    m_history = NULL;
    //Fonero::WalletManagerFactory::getWalletManager()->closeWallet(m_walletImpl);
