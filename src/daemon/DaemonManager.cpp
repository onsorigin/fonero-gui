...
{
    // prepare command line arguments and pass to fonero-daemon
    ...
    qDebug() << "starting fonero-daemon " + m_fonero_daemon;
    ...
    // Start fonero-daemon
    bool started = m_daemon->startDetached(m_fonero_daemon, arguments);
    ...
    while(true && !m_app_exit) {
        ...
        if(running(testnet)) {
            ...
            if(counter >= 5) {
                qDebug() << "Killing it! ";
#ifdef Q_OS_WIN
                QProcess::execute("taskkill /F /IM fonero-daemon.exe");
#else
                QProcess::execute("pkill fonero-daemon");
#endif
            }
...
{ 
    ...
    qDebug() << status;
    // `./fonero-daemon status` returns BUSY when syncing.
...
bool DaemonManager::sendCommand(const QString &cmd,bool testnet, QString &message) const
{
    ...
    p.start(m_fonero_daemon, external_cmd);
    ...
    : QObject(parent)
{
    ...
    // Platform depetent path to fonero-daemon
#ifdef Q_OS_WIN
    m_fonero_daemon = QApplication::applicationDirPath() + "/fonero-daemon.exe";
#elif defined(Q_OS_UNIX)
    m_fonero_daemon = QApplication::applicationDirPath() + "/fonero-daemon";
#endif

    if (m_fonero_daemon.length() == 0) {
