...
[Setup]
AppName=Fonero GUI Wallet
; For InnoSetup this is the property that uniquely identifies the application as such
; Thus it's important to keep this stable over releases
; With a different "AppName" InnoSetup would treat a mere update as a completely new application and thus mess up

AppVersion=0.11.1.0
DefaultDirName={pf}\Fonero GUI Wallet
DefaultGroupName=Fonero GUI Wallet
UninstallDisplayIcon={app}\fonero-wallet-gui.exe
...
; Fonero GUI wallet
Source: "bin\fonero-wallet-gui.exe"; DestDir: "{app}"; Flags: comparetimestamp

; Fonero GUI wallet log file
...
Source: "fonero-wallet-gui.log"; DestDir: "{app}"; Flags: comparetimestamp; Permissions: users-modify

; Fonero CLI wallet
Source: "bin\fonero-wallet-cli.exe"; DestDir: "{app}"; Flags: comparetimestamp

; Fonero wallet RPC interface implementation
Source: "bin\fonero-wallet-rpc.exe"; DestDir: "{app}"; Flags: comparetimestamp

; Fonero daemon
Source: "bin\fonero-daemon.exe"; DestDir: "{app}"; Flags: comparetimestamp

; Fonero daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "fonero-daemon.bat"; DestDir: "{app}"; Flags: comparetimestamp;

; Fonero blockchain utilities
Source: "bin\fonero-blockchain-export.exe"; DestDir: "{app}"; Flags: comparetimestamp
Source: "bin\fonero-blockchain-import.exe"; DestDir: "{app}"; Flags: comparetimestamp

; was present in 0.10.3.1, not present anymore in 0.11.1.0
; Source: "bin\fonero-utils-deserialize.exe"; DestDir: "{app}"; Flags: comparetimestamp
...
; Filename: "{app}\fonero-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  blockChainDefaultDir := ExpandConstant('{commonappdata}\bitfonero');
  s := 'The default folder to store the Fonero blockchain is ' + blockChainDefaultDir;
...
begin
  Result := BlockChainDir('') + '\bitfonero.log';
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "fonero-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the Fonero daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\fonero-daemon.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\fonero-daemon.bat'), s, false); 
  end;
end;
...
[Icons]
; Icons in the "Fonero GUI Wallet" program group
; Windows will almost always display icons in alphabetical order, per level, so specify the text accordingly
Name: "{group}\GUI Wallet"; Filename: "{app}\fonero-wallet-gui.exe"
...
Name: "{group}\Utilities\Fonero Daemon"; Filename: "{app}\fonero-daemon.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\fonero-wallet-cli.exe"; WorkingDir: "{userdocs}\Fonero\wallets"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: "{userdocs}\Fonero\wallets"
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: "{app}\fonero-wallet-gui.log"
Name: "{group}\Utilities\x (Try Daemon, Exit Confirm)"; Filename: "{app}\fonero-daemon.bat"
Name: "{group}\Utilities\x (Try GUI Wallet Low Graphics Mode)"; Filename: "{app}\start-low-graphics-mode.bat"
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM fonero-daemon.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{userdesktop}\GUI Wallet"; Filename: "{app}\fonero-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\fonero-project"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\fonero-project\fonero-gui"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\fonero-project\fonero-gui"; ValueType: string; ValueName: "daemonFlags"; ValueData: {code:DaemonFlags};
